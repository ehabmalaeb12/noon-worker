...
const NOON_WORKER = "https://noon-worker.ehabmalaeb2.workers.dev"; // NOON - added
...
/* ---------------- NOON (NEW - faster first than Sharaf) ---------------- */
async function fetchNoon(query) {
  try {
    const url = `${NOON_WORKER}/search?q=${encodeURIComponent(query)}`;
    const res = await fetchWithRetry(url, { headers: { "Accept": "application/json" } }, 2, 300);
    const data = await res.json();
    log(`Noon results count ${data.results?.length || 0}`);
    // Normalize results if needed: ensure { title, price, currency, image, link, store }
    ...
    log("Noon fetch failed", e?.message || e);
    return [];
  }
}
...
async function startSearch() {
  ...
  // 2) Noon (moved before Sharaf as requested)
  try {
    const noonResults = await fetchNoon(query);
    if (noonResults && noonResults.length) {
      noonResults.forEach(r => renderProduct(r));
    }
  } catch (e) {
    log("Noon: unexpected error", e);
  }

  // 3) Sharaf links -> product details (safe, limited concurrency)
